...
func (h *ProductHandlers) CreateOrder(c *gin.Context) {
	...
	for _, id := range req.CartItemIDs {
		...
		cartItems = append(cartItems, product.CartItem{CartItemID: id, ProductID: id}) // สมมุติว่า ProductID ใช้จาก CartItemID
	}
...
func (h *ProductHandlers) UpdateCartItemtatusHandler(c *gin.Context) {
	var input struct {
		OrderID  int    `json:"order_id"`
		SellerID int    `json:"seller_id"` // เพิ่ม seller_id
	...
	err := h.store.UpdateCartItemStatus(c.Request.Context(), input.OrderID, input.SellerID, input.Status)
	if err != nil {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Cart item status updated successfully"})
}

func (h *ProductHandlers) GetOrdersSort(c *gin.Context) {
	// รับค่าพารามิเตอร์สถานะจาก query string (ตัวอย่างเช่น ?status=ทั้งหมด)
	status := c.Param("status") // ถ้าไม่มีค่า status จะใช้ค่า "" แทน
...
func (h *ProductHandlers) UpdateUserContactHandler(c *gin.Context) {
	...
	var req struct {
		...
		DisplayName string `json:"display_name"` // ไม่บังคับ
		Address     string `json:"address"`      // ไม่บังคับ
		Phone       string `json:"phone"`        // ไม่บังคับ
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request payload"})
		return
